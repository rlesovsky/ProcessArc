...
import { screen as screenContainer } from '@/lib/layout';
...
export function readWizardDemoOverride(): Partial<WizardState> | null {
...
export interface WizardTabExposed {
  /** The current site/project name, so the app header can show it. */
  projectName: string;
  /** Source filename for the header. */
  sourceFilename: string;
  /** ERP number for the header. */
  erpNumber?: string;
}

interface WizardTabProps {
  /** Called when header-display fields change, so the outer shell stays in sync. */
  onHeaderInfoChange?: (info: WizardTabExposed) => void;
}

export function WizardTab({ onHeaderInfoChange }: WizardTabProps) {
  const [s, setS] = useState<WizardState>(() => ({ ...INITIAL, ...(readWizardDemoOverride() ?? {}) }));

  // Surface header info (project name, source filename, ERP number) up
  // to the app shell so the header band can show project context even
  // while the user is on a different tab.
  const erpNumber = s.plant?.erp_number;
  useEffect(() => {
  ...
  }, [onHeaderInfoChange, s.projectName, s.sourceFilename, erpNumber]);
  ...
  if (s.stage === 'configure') {
    ...
    // The Review screen's own Save & Continue button drives the advance.
    canContinue = false;
